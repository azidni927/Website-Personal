...
function openModal(id) {
  ...
  if (mi) {
    mi.innerHTML = p.img ? `<img src="${p.img}" alt="" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><div class="moji" style="display:none">${p.cat === 'Getuk Goreng' ? '🍠' : p.cat === 'Nopia' ? '🟤' : '🌽'}</div>` : `<div class="moji">${p.cat === 'Getuk Goreng' ? '🍠' : p.cat === 'Nopia' ? '🟤' : '🌽'}</div>`;
  }
